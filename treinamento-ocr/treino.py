...
#Adiciona uma imagem vazia (toda preta) em X e atribui a classe "fundo" em Y para esta imagem
...
if len(im_folder) < 1:
    for c in range(48,91):
        if c < 58 or c > 64:
            os.mkdir(f"ocr-treino/images/{str(chr(c))}")
	...
	if c < 58 or c > 64: 
		...
		for i in range(40,45):
			for fontpath in fonts: 
				...
				font = ImageFont.truetype("ocr-treino/fonts/"+fontpath, i)
				...
				cv2.imwrite('ocr-treino/images/' + str(chr(c)) + "/" + str(chr(c)) + str(count)+".jpg", img)  
				print("Salvando: " + 'ocr-treino/images/' + str(chr(c)) + "/" + str(chr(c)) + str(count)+".jpg") 
...
#Salva modelo
joblib.dump((clf, ss), "ocr-treino/caracteres.pkl", compress=3)
print("modelo salvo")